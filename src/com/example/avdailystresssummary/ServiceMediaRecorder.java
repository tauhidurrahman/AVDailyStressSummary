...
public class ServiceMediaRecorder extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		mFileName += "/dailyStressAudio.wav";
		// setupRecording();
		// Toast.makeText(getBaseContext(),
		// "onStartCommand ServiceAudioCapture", Toast.LENGTH_LONG).show();
		rHandler.removeCallbacks(playAudio);
		rHandler.postDelayed(playAudio, 1);// <<-temporarily
	...
	public Runnable recordAudio = new Runnable() {
		public void run() {
			if (startRecordings == true) {
				...
				rHandler.postDelayed(recordAudio, audioLengthSecond * 1000);// register
																			// again
																			// to
																			// start
																			// after
	...
	public Runnable playAudio = new Runnable() {
		public void run() {
			if (startPlaying == true) {
				startPlaying();
				startPlaying = false;
				pHandler.postDelayed(playAudio, audioLengthSecond * 1000);// register
																			// again
																			// to
																			// start
																			// after
	...
	public void onDestroy() {
		super.onDestroy();
		Toast.makeText(getBaseContext(), "onDestroy ServiceAudioCapture",
	...
	private void startRecording() {
		...
		mRecorder.start();
		Toast.makeText(getBaseContext(), "Starting Recording",
				Toast.LENGTH_LONG).show();
	...
	private void stopRecording() {
		...
		mRecorder.release();
		mRecorder = null;
		Toast.makeText(getBaseContext(), "Stopping ServiceAudioCapture",
				Toast.LENGTH_LONG).show();
	}
}
